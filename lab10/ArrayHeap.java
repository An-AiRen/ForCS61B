...
public class ArrayHeap<T> implements ExtrinsicPQ<T> {
    ...
    private void swim(int index) {
        ...
        // remember index 1 store the information of root.
        if (index == 1 || contents[parentIndex(index)].priority() < contents[index].priority())
            return;
    ...
    private void sink(int index) {
        ...
        if (!inBounds(temp) || contents[index].priority() < contents[temp].priority()) { // reach the bottom of the PQ.
            return;
        }
        swap(index, temp);
        sink(temp);
    ...
    public void insert(T item, double priority) {
        ...
        if (size + 1 == contents.length) { // because index 0 is useless, we start by 1.
